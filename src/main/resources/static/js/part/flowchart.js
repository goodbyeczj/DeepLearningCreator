            ...
            success:function(data, textStatus){
                if(textStatus=="success"){
                    ...
                    $('#fullscreen').modal({
                        ...
                        duration: {},
                        onShow: function() {
                            $('#run_core pre').append(data.core.toString());
                        },
                        onHidden: function() {
    ...
    $('.editor-toolbar .icon.paste').on('click', function(e) {
        $('.ui.modal.prop_node').modal({
            ...
            onApprove: function() {
                ...
                $.ajax({
                    url:'/model/commitData.do', async:true,
                    type:'post',
                    data: {str:str},
                    // dataType:'json',
                    beforeSend:function(jqXHR, settings){
...
$(function () {
    $('.editor-toolbar .icon.repeat').click(function () {
        window.location.href="/model/to_classic_net.do";
    });
});
$(function () {
    $('#download').click(function () {
        downloadFile("/model/download.do");
        downloadFile("/model/downloadData.do");
        downloadFile("/model/downloadPredict.do");
    });
});
//下载文件
// 定义到jQuery全局变量下-文件下载

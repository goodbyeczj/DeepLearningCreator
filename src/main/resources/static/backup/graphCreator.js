...
document.onload = (function(d3, saveAs, Blob, vkbeautify) {
  ...
  GraphCreator.prototype.handleClickZoom = function() {
    ...
    GraphCreator.prototype.filterDense = function() {
        var thisGraph = this;
        var dense = thisGraph.nodes.filter(function(node) {
            return node.component == 'Dense';
        });
        return dense;
    };
    ...
    GraphCreator.prototype.spliceLinksForNode = function(node) {
        ...
        toSplice.map(function(l) {
            var source = l.source.conventional.current;
            var target = l.target.conventional.current;
    ...
    GraphCreator.prototype.circleMouseUp = function(d3node, d) {
        ...
        if (mouseDownNode !== d) {
            ...
            if (!filtRes[0].length) {
                mouseDownNode.conventional.out += ','+newEdge.target.conventional.current;
                var output = mouseDownNode.conventional.out.toString();
                if (output.charAt(0)==","){
                    output = output.substring(1,output.length);
                }
                mouseDownNode.conventional.out = output;
                d.conventional.in += ','+newEdge.source.conventional.current;
                var input = d.conventional.in.toString();
                if (input.charAt(0)==","){
                    input = input.substring(1,input.length);
                }
                d.conventional.in = input;
    ...
    GraphCreator.prototype.showMenu = function() {
        ...
        d3.select("#rMenu").style({
            "top": (d3.event.clientY-42)+"px",
            "left": (d3.event.clientX-165)+"px",
            "display": "block"
        });
    ...
    GraphCreator.prototype.svgMouseUp = function() {
        ...
        if (state.justScaleTransGraph) {
            ...
            thisGraph.updateGraph();
        } else if (state.shiftNodeDrag &&state.mouseDownNode!=null) {
            // dragged from node
            var node ={};
            var title = state.mouseDownNode.title;
            var component = state.mouseDownNode.component;
            var type = state.mouseDownNode.type;
            switch (title) {
                case '普通活动':
                    node = {
                        id: seqer_nodeID.gensym(),
